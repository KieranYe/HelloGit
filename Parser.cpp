...
* 返回表达式的计算结果
*/
double Compute(string expr, double argx)
{
	double x = argx;               //x变量
	...
	for (string::size_type i(0); i < eLen; ++i)
	{
		...
		//这里来判断是否读入了一个数字（的一部分）
		if (isdigit(expr[i]) || expr[i] == '.' || expr[i] == 'x') isDigit = true;
		else isDigit = false;
		//这里判断是否读入了一个函数（的一部分）
		if (islower(expr[i]) && expr[i] != 'x') isFun = true;
		else isFun = false;
		//这里判断当前位置是否为右括号
		...
		{
			if (!dTmp.empty() && dTmp != "x")
				rExpr.push(strtod(dTmp.c_str(), 0));	//上一个数字入表达式栈
			else if (dTmp == "x")
				rExpr.push(x);
			dTmp = "";
			if (expr[i] == ')')
